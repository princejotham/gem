...
function required() {
    ...
    $CI->load->model('account_/account__model');
    ...
    if ($RTR->class != "frontend" && $RTR->class != "request" && $RTR->class != "auth") {
        if (!$CI->ion_auth->in_group(array('superadmin'))) {
            if ($CI->ion_auth->in_group(array('admin'))) {
                ...
                $CI->account__id = $CI->db->get_where('account_', array('ion_user_id' => $current_user_id))->row()->id;

                if (!empty($CI->account__id)) {
                    $newdata = array(
                        'account__id' => $CI->account__id,
                    );
                ...
                $CI->account__id = $CI->db->get_where($group_name, array('ion_user_id' => $current_user_id))->row()->account__id;
                if (!empty($CI->account__id)) {
                    $newdata = array(
                        'account__id' => $CI->account__id,
                    );
    ...
    if ($RTR->class != "frontend" && $RTR->class != "request" && $RTR->class != "auth") {
        if (!$CI->ion_auth->in_group(array('superadmin'))) {
            if ($CI->ion_auth->in_group(array('admin'))) {
                $current_user_id = $CI->ion_auth->user()->row()->id;
                $modules = $CI->db->get_where('account_', array('ion_user_id' => $current_user_id))->row()->module;
                ...
                $group_name = $CI->db->get_where('groups', array('id' => $group_id))->row()->name;
                $group_name = strtolower($group_name);
                $account__id = $CI->db->get_where($group_name, array('ion_user_id' => $current_user_id))->row()->account__id;
                $modules = $CI->db->get_where('account_', array('id' => $account__id))->row()->module;
                $CI->modules = explode(',', $modules);
            }
        }
    }
    ...
    if (!in_array($RTR->class, $common)) {
        if (!$CI->ion_auth->in_group(array('superadmin'))) {
            ...
            } elseif (!in_array('patient_care', $CI->modules)) {
                redirect('home');
            }
        }
    }
