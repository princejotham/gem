    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">  <?php echo lang('update_contact_summary'); ?></h4>
            </div>
            <div class="modal-body">
                <form role="form" id="editBloodBank" class="clearfix" action="contact_list/updateBloodBank" method="post" enctype="multipart/form-data">
...
<script type="text/javascript">
                        $(document).ready(function () {
                            $(".editbutton").click(function (e) {
                                ...
                                $('#myModal2').modal('show');
                                $.ajax({
                                    url: 'contact_list/updateBloodBankByJason?id=' + iid,
                                    method: 'GET',
                                    data: '',
                                    dataType: 'json',
                                }).success(function (response) {
                                    // Populate the form fields with the data returned from server
                                    $('#editBloodBank').find('[name="id"]').val(response.contactsummary.id).end()
                                    $('#editBloodBank').find('[name="group"]').val(response.contactsummary.group).end()
                                    $('#editBloodBank').find('[name="status"]').val(response.contactsummary.status).end()
