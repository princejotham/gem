...
class Patient extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Health_worker', 'Patient', 'Case_manager', 'Laboratorist', 'Finance_', 'Admin_comm'))) {
    ...
    public function addNew() {
        ...
        // Validating Phone Field           
        $this->form_validation->set_rules('contactgroup', 'Contact Group', 'trim|min_length[1]|max_length[10]|xss_clean');
    ...
    function getPatientByJason() {
        ...
        $case_manager = $data['patient']->case_manager;
        $data['case_manager'] = $this->case_manager_model->getCase_managerById($case_manager);
...
// Validating Deposited Amount Field
    ...
    function medicalHistory() {
        ...
        $patient_account__id = $this->patient_model->getPatientById($id)->account__id;
        ...
        $data['patient'] = $this->patient_model->getPatientById($id);
        $data['patient_cares'] = $this->patient_care_model->getPatient_careByPatient($data['patient']->id);
        $data['patients'] = $this->patient_model->getPatient();
        $data['case_managers'] = $this->case_manager_model->getCase_manager();
        ...
        foreach ($data['patient_cares'] as $patient_care) {
            $case_manager_details = $this->case_manager_model->getCase_managerById($patient_care->case_manager);
            if (!empty($case_manager_details)) {
                $case_manager_name = $case_manager_details->name;
            } else {
                $case_manager_name = '';
            }
            $timeline[$patient_care->date + 1] = '<div class="panel-body profile-activity" >
                <h5 class="pull-left"><span class="label pull-right r-activity">' . lang('patient_care') . '</span></h5>
                                            <h5 class="pull-right">' . date('d-m-Y', $patient_care->date) . '</h5>
                                                ...
                                                <div class="activity-desk">
                                                    <div class="panel col-md-6">
                                                        <div class="panel-body">
                                                            ...
                                                            <h4>' . date('d-m-Y', $patient_care->date) . '</h4>
                                                            <p></p>
                                                            <i class=" fa fa-user-md"></i>
                                                                <h4>' . $case_manager_name . '</h4>
                                                                    <p></p>
                                                                    <i class=" fa fa-clock-o"></i>
                                                                <p>' . $patient_care->s_time . ' - ' . $patient_care->e_time . '</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
        ...
        foreach ($data['prescriptions'] as $prescription) {
            $case_manager_details = $this->case_manager_model->getCase_managerById($prescription->case_manager);
            if (!empty($case_manager_details)) {
                $case_manager_name = $case_manager_details->name;
            } else {
                $case_manager_name = '';
        ...
        foreach ($data['labs'] as $lab) {
            ...
            $case_manager_details = $this->case_manager_model->getCase_managerById($lab->case_manager);
            if (!empty($case_manager_details)) {
                $lab_case_manager = $case_manager_details->name;
            } else {
                $lab_case_manager = '';
            }
        ...
        foreach ($data['patient_materials'] as $patient_material) {
            $timeline[$patient_material->date + 5] = '<div class="panel-body profile-activity" >
                                           <h5 class="pull-left"><span class="label pull-right r-activity">' . lang('records') . '</span></h5>
    ...
    function getPatientByPatient_careByDctorId($case_manager_id) {
        $data = array();
        $patient_cares = $this->patient_care_model->getPatient_careByCase_manager($case_manager_id);
        foreach ($patient_cares as $patient_care) {
            $patient_exists = $this->patient_model->getPatientById($patient_care->patient);
            if (!empty($patient_exists)) {
                $patients[] = $patient_care->patient;
            }
        }
    ...
    function getPatient() {
        ...
        foreach ($data['patients'] as $patient) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Finance_', 'Admin_comm', 'Laboratorist', 'Health_worker', 'Case_manager'))) {
            ...
            if ($this->ion_auth->in_group(array('Laboratorist', 'Health_worker', 'Case_manager'))) {
                $info[] = array(
                    $patient->id,
                    $patient->name,
                    $patient->phone,
    ...
    function getRecords() {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['records'] = $this->patient_model->getDocumentBySearch($search);
            } else {
                $data['records'] = $this->patient_model->getPatientMaterial();
            }
        } else {
            if (!empty($search)) {
                $data['records'] = $this->patient_model->getDocumentByLimitBySearch($limit, $start, $search);
            } else {
                $data['records'] = $this->patient_model->getDocumentByLimit($limit, $start);
            }
        }
        //  $data['patients'] = $this->patient_model->getPatient();
        ...
        foreach ($data['records'] as $record) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Finance_', 'Admin_comm', 'Laboratorist', 'Health_worker', 'Case_manager'))) {
                //   $options1 = '<a type="button" class="btn editbutton" title="Edit" data-toggle="modal" data-id="463"><i class="fa fa-edit"> </i> Edit</a>';
                $options1 = '<a class="btn btn-info btn-xs" href="' . $record->url . '" download> ' . lang('download') . ' </a>';
            }
            if ($this->ion_auth->in_group(array('admin', 'Finance_', 'Admin_comm', 'Laboratorist', 'Health_worker', 'Case_manager'))) {
                $options2 = '<a class="btn btn-info btn-xs delete_button" href="patient/deletePatientMaterial?id=' . $record->id . '&redirect=records"onclick="return confirm(\'You want to delete the item??\');"> X </a>';
            }

            if (!empty($record->patient)) {
                $patient_info = $this->patient_model->getPatientById($record->patient);
                if (!empty($patient_info)) {
                    $patient_details = $patient_info->name . '</br>' . $patient_info->address . '</br>' . $patient_info->phone . '</br>';
                } else {
                    $patient_details = $record->patient_name . '</br>' . $record->patient_address . '</br>' . $record->patient_phone . '</br>';
                }
            } else {
                $patient_details = '';
            }
            ...
            $info[] = array(
                ...
                $patient_details,
                $record->title,
                '<img src="' . $record->url . '" width="100px" height="100px">',
                $options1 . ' ' . $options2
                    // $options4
            );
        }
