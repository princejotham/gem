...
class Patient_model extends CI_model {
    ...
    function getLimit() {
        $current = $this->db->get_where('patient', array('account__id' => $this->session->userdata('account__id')))->num_rows();
        $limit = $this->db->get_where('account_', array('id' => $this->session->userdata('account__id')))->row()->p_limit;
        if(!is_numeric($limit)){
            $limit = 0;
        }
