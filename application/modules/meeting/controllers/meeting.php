...
class Meeting extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Case_manager', 'Patient'))) {
    ...
    function live() {
        ...
        $case_manager_ion_id = $live_details->case_manager_ion_id;
        $settings = $this->meeting_model->getMeetingSettingsById($case_manager_ion_id);
    ...
    function instantLive() {
        ...
        $patient_care_id = $this->input->get('id');
        $patient_care_details = $this->patient_care_model->getPatient_careById($patient_care_id);

        $patient = $patient_care_details->patient;
        ...
        if ($this->ion_auth->in_group(array('Case_manager'))) {
            ...
            $case_manager_details = $this->case_manager_model->getCase_managerByIonUserId($case_manager_ion_id);
            $case_manager = $case_manager_details->id;
        } else {
            $case_manager = $patient_care_details->case_manager;
            $case_manager_details = $this->case_manager_model->getCase_managerById($case_manager);
            $case_manager_ion_id = $case_manager_details->ion_user_id;
        }

        $case_managername = $case_manager_details->name;

        if ($this->ion_auth->in_group(array('Case_manager'))) {
            if ($case_manager != $patient_care_details->case_manager) {
                redirect('home/permission');
            }
        }
        ...
        $data = array(
            ...
            'case_managername' => $case_managername,
            'case_manager_ion_id' => $case_manager_ion_id,
            'topic' => lang('case_manager') . ' ' . lang('patient_care'),
            'type' => 2,
        ...
        if (!empty($response->id)) {
            ...
            //  $this->sendSmsDuringMeeting($patient, $case_manager, $start_date);
...
//API Requests Starts
    public function getMeetingsByMeetingId($meeting_id) {
        $start_time = NULL;
        $data = array();
        $case_manager_ion_id = $this->meeting_model->getMeetingByZoomMeetingId($meeting_id)->case_manager_ion_id;
        $data['case_manager_ion_id'] = $case_manager_ion_id;
    ...
    public function createAMeeting($data = array(), $meeting_id) {
        ...
        $createAMeetingArray['case_manager_ion_id'] = $data['case_manager_ion_id'];
    ...
    function generateJWT($data = array()) {
        $settings = $this->meeting_model->getMeetingSettingsById($data['case_manager_ion_id']);
    ...
    function settings() {
        ...
        $this->form_validation->set_rules('api_key', 'Api Key', 'trim|min_length[1]|max_length[100]|xss_clean');
        // Validating SMS Field
        $this->form_validation->set_rules('authkey', 'Api Secret', 'trim|min_length[1]|max_length[100]|xss_clean');
        // Validating SMS Field

        if ($this->form_validation->run() == FALSE) {
            $data = array();
            $data['settings'] = $this->meeting_model->getMeetingSettingsById($id);
...
// API Requests Ends
    ...
    function calendar() {
        ...
        if ($this->ion_auth->in_group(array('Case_manager'))) {
            ...
            $data['meetings'] = $this->meeting_model->getMeetingByCase_manager($case_manager);
        } else {
    ...
    public function addNew() {
        ...
        if (empty($id)) {
            ...
            $case_manager = $this->meeting_model->getMeetingById($id)->case_manager;
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $case_managername = $this->case_manager_model->getCase_managerById($case_manager)->name;
            if ($this->ion_auth->in_group('Case_manager')) {
                $case_manager_ion_id = $this->ion_auth->get_user_id();
            } else {
                $case_manager_ion_id = $this->case_manager_model->getCase_managerById($case_manager)->ion_user_id;
            }
    ...
    function sendSmsDuringMeeting($patient, $case_manager, $start_time) {
        //sms
        ...
        $data1 = array(
            ...
            'account__name' => $set['settings']->system_vendor
        );
    ...
    function getMeetingByJasonByCase_manager() {
        $id = $this->input->get('id');
        $query = $this->meeting_model->getMeetingByCase_manager($id);
        $jsonevents = array();
    ...
    function getMeetingByJason() {
        ...
        if ($this->ion_auth->in_group(array('Case_manager'))) {
            $case_manager_ion_id = $this->ion_auth->get_user_id();
            $case_manager = $this->db->get_where('case_manager', array('ion_user_id' => $case_manager_ion_id))->row()->id;
            $query = $this->meeting_model->getMeetingByCase_manager($case_manager);
    ...
    function getMeetingByCase_managerId() {
        $id = $this->input->get('id');
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('meeting_by_case_manager', $data);
        $this->load->view('home/footer'); // just the header file
    }
    ...
    function editMeeting() {
        ...
        $data['case_managers'] = $this->case_manager_model->getCase_managerById($data['meeting']->case_manager);
        $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('add_new', $data);
        $this->load->view('home/footer'); // just the footer file 
    }
    ...
    function editMeetingByJason() {
        ...
        $data['case_manager'] = $this->case_manager_model->getCase_managerById($data['meeting']->case_manager);
        echo json_encode($data);
    }
    ...
    function delete() {
        ...
        if (!empty($case_manager_id)) {
            redirect('meeting/getMeetingByCase_managerId?id=' . $case_manager_id);
        } else {
            redirect('meeting/upcoming');
        }
    }
    ...
    function getMeetingList() {
        ...
        foreach ($data['meetings'] as $meeting) {
            ...
            if ($status->status == 'started') {
                ...
                $case_managerdetails = $this->case_manager_model->getCase_managerById($meeting->case_manager);
                if (!empty($case_managerdetails)) {
                    $case_managername = $case_managerdetails->name;
                } else {
                    $case_managername = ' ';
                }
        ...
        foreach ($data['meetings'] as $meeting) {
            ...
            if (strtotime($meeting->start_time) > time()) {
                ...
                if (!empty($case_managerdetails)) {
                    ...
                    $case_managername = $meeting->case_managername;
                }
