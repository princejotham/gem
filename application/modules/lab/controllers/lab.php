...
class Lab extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Finance_', 'Admin_comm', 'Health_worker', 'Laboratorist', 'Case_manager', 'Patient'))) {
    ...
    public function index() {
        ...
        if (!empty($id)) {
            ...
            if ($lab_details->account__id != $this->session->userdata('account__id')) {
                redirect('home/permission');
            }
        }
...
// Adding New Patient
    ...
    function editLab() {
        if ($this->ion_auth->in_group(array('admin', 'Case_manager', 'Laboratorist', 'Health_worker', 'Patient'))) {
            $data = array();
            $data['settings'] = $this->settings_model->getSettings();
            $data['categories'] = $this->lab_model->getLabCategory();
            $data['patients'] = $this->patient_model->getPatient();
...
// Validating Price Field
    ...
    function editTemplate() {
        if ($this->ion_auth->in_group(array('admin', 'Case_manager', 'Laboratorist', 'Health_worker', 'Patient'))) {
            ...
            $template_details = $this->lab_model->getTemplateById($id);
            if ($template_details->account__id != $this->session->userdata('account__id')) {
                redirect('home/permission');
            }
...
// Validating Description Field
    ...
    function invoice() {
        ...
        if ($data['lab']->account__id != $this->session->userdata('account__id')) {
...
// } 
    ...
    function getLab() {
        ...
        foreach ($data['labs'] as $lab) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Laboratorist', 'Case_manager'))) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Case_manager', 'Laboratorist'))) {
            ...
            $case_manager_info = $this->case_manager_model->getCase_managerById($lab->case_manager);
            if (!empty($case_manager_info)) {
                $case_manager = $case_manager_info->name;
            } else {
                if (!empty($lab->case_manager_name)) {
                    $case_manager = $lab->case_manager_name;
                } else {
                    $case_manager = ' ';
                }
            }
