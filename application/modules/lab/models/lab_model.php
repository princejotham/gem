...
class Lab_model extends CI_model {
    ...
    function getLabBySearch($search) {
        ...
        $query = $this->db->select('*')
                ...
                ->where("(id LIKE '%" . $search . "%' OR patient_name LIKE '%" . $search . "%' OR patient_phone LIKE '%" . $search . "%' OR patient_address LIKE '%" . $search . "%'OR case_manager_name LIKE '%" . $search . "%'OR date_string LIKE '%" . $search . "%')", NULL, FALSE)
                ->get();
    ...
    function getLabByCase_manager($case_manager) {
        $this->db->select('*');
        $this->db->from('lab');
    ...
    function getLabByCase_managerDate($case_manager, $date_from, $date_to) {
        $this->db->where('account__id', $this->session->userdata('account__id'));
        $this->db->select('*');
        $this->db->from('lab');
        $this->db->where('case_manager', $case_manager);
