                ...
                <div class="adv-table editable-table">
                    ...
                    <table class="table table-striped table-hover table-bordered" id="editable-sample">
                        ...
                        foreach ($holidays as $holiday) {
                            ...
                            <tr class="">
                                <td> <?php echo $i; ?></td>
                                <td> <?php echo $this->case_manager_model->getCase_managerById($holiday->case_manager)->name; ?></td> 
                                ...
                                <?php if ($this->ion_auth->in_group(array('admin', 'Case_manager'))) { ?>
                                    <td>
                                        ...
                                        <a class="btn btn-info btn-xs btn_width delete_button" href="schedule/deleteHoliday?id=<?php echo $holiday->id; ?>&case_manager=<?php echo $holiday->case_manager; ?>&redirect=schedule/allHolidays" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fa fa-trash-o"> </i> <?php echo lang('delete'); ?></a>
                                    </td>
                                <?php } ?>
                            </tr>
                        <?php } ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="editHolidayForm" class="clearfix" action="schedule/addHoliday" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group col-md-6 row">
                        <label for="exampleInputEmail1">  <?php echo lang('case_manager'); ?></label>
                        <select class="form-control m-bot15 js-example-basic-single case_manager" name="case_manager" value=''>  
                            <option value="">Select .....</option>
                            <?php foreach ($case_managers as $case_manager) { ?>
                                <option value="<?php echo $case_manager->id; ?>"<?php
                                if (!empty($holiday->case_manager)) {
                                    if ($holiday->case_manager == $case_manager->id) {
                                        echo 'selected';
                                    }
                                }
                                    ...
                                    $(document).ready(function () {
                                        $(".editbutton").click(function (e) {
                                            ...
                                            $.ajax({
                                                ...
                                                $('.js-example-basic-single.case_manager').val(response.holiday.case_manager).trigger('change');
