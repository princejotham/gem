...
class Finance_model extends CI_model {
    ...
    function getPaymentBySearch($search) {
        ...
        /*
          ...
          $this->db->or_like('case_manager_name', $search);
        ...
        $query = $this->db->select('*')
                ...
                ->where("(id LIKE '%" . $search . "%' OR amount LIKE '%" . $search . "%' OR gross_total LIKE '%" . $search . "%' OR patient_name LIKE '%" . $search . "%'OR patient_phone LIKE '%" . $search . "%'OR patient_address LIKE '%" . $search . "%'OR remarks LIKE '%" . $search . "%'OR case_manager_name LIKE '%" . $search . "%'OR flat_discount LIKE '%" . $search . "%'OR date_string LIKE '%" . $search . "%')", NULL, FALSE)
                ->get();
    ...
    function thisMonthPatient_care() {
        $this->db->where('account__id', $this->session->userdata('account__id'));
    ...
    function thisMonthPatient_careTreated() {
        $this->db->where('account__id', $this->session->userdata('account__id'));
        $query = $this->db->get('patient_care')->result();
        $total = array();
        foreach ($query as $q) {
    ...
    function thisMonthPatient_careCancelled() {
        $this->db->where('account__id', $this->session->userdata('account__id'));
        $query = $this->db->get('patient_care')->result();
        $total = array();
        foreach ($query as $q) {
    ...
    function getCase_managerCommissionByCategory($id) {
        $this->db->where('account__id', $this->session->userdata('account__id'));
        $this->db->where('id', $id);
        $query = $this->db->get('payment_category');
        return $query->row();
    ...
    function getPaymentByCase_manager($case_manager) {
        $this->db->where('account__id', $this->session->userdata('account__id'));
        $this->db->select('*');
    ...
    function getPaymentByCase_managerDate($case_manager, $date_from, $date_to) {
        $this->db->where('account__id', $this->session->userdata('account__id'));
        $this->db->select('*');
        $this->db->from('payment');
        $this->db->where('case_manager', $case_manager);
    ...
    function getThisMonth() {
        ...
        $patient_cares = $this->db->get('patient_care')->result();
        foreach ($patient_cares as $patient_care) {
            if (date('m/y', $patient_care->date) == date('m/y', time())) {
                $this_month_patient_care[] = 1;
            }
        }

        if (!empty($this_month_patient_care)) {
            $this_month_patient_care = array_sum($this_month_patient_care);
        } else {
            $this_month_patient_care = 0;
        }

        $this_month_details = array($this_month_payment, $this_month_expense, $this_month_patient_care);
