...
class Finance extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('finance_/finance__model');
        $this->load->model('admin_comm/admin_comm_model');
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Finance_', 'Admin_comm', 'Health_worker', 'Laboratorist', 'Case_manager'))) {
...
// Adding New Patient
                if ($this->ion_auth->email_check($d_email)) {
                    ...
                    $this->case_manager_model->insertCase_manager($data_d);
                    $case_manager_user_id = $this->db->get_where('case_manager', array('email' => $d_email))->row()->id;
                    $id_info = array('ion_user_id' => $ion_user_id);
            ...
            if ($case_manager == 'add_new') {
                $case_manager = $case_manager_user_id;
            }
            ...
            if (!empty($case_manager)) {
                ...
                foreach ($all_cat_name as $indiviual_cat_nam) {
                    ...
                    $account__amount_per_unit = $indiviual_cat_nam1[1] * $h_commission / 100;
                    $account__amount_by_category[] = $account__amount_per_unit * $qty;
                }
                $account__amount = array_sum($account__amount_by_category);
                if ($discount_type == 'flat') {
                    $flat_discount = $discount;
                    $gross_total = $sub_total - $flat_discount;
                    $case_manager_amount = $amount - $account__amount - $flat_discount;
                } else {
                ...
                $case_manager_amount = '0';
                if ($discount_type == 'flat') {
                    $flat_discount = $discount;
                    $gross_total = $sub_total - $flat_discount;
                    $account__amount = $gross_total;
                } else {
            ...
            if (!empty($case_manager)) {
                ...
                $case_manager_name = $case_manager_details->name;
            } else {
                $case_manager_name = 0;
            }
            ...
            if (empty($id)) {
                $data = array(
                    ...
                    'status' => 'unpaid',
                    'account__amount' => $account__amount,
                    'case_manager_amount' => $case_manager_amount,
                    'user' => $user,
    ...
    function editPayment() {
        if ($this->ion_auth->in_group(array('admin', 'Finance_', 'Admin_comm'))) {
            ...
            if (!empty($id)) {
                ...
                if ($payment_details->account__id != $this->session->userdata('account__id')) {
                    redirect('home/permission');
                }
            }
    ...
    public function addOtPayment() {
        $id = $this->input->post('id');
        $patient = $this->input->post('patient');
        $case_manager_c_s = $this->input->post('case_manager_c_s');
        $case_manager_a_s_1 = $this->input->post('case_manager_a_s_1');
        $case_manager_a_s_2 = $this->input->post('case_manager_a_s_2');
        $case_manager_anaes = $this->input->post('case_manager_anaes');
...
// Validating Consultant surgeon Field
        $this->form_validation->set_rules('case_manager_c_s', 'Consultant surgeon', 'trim|min_length[1]|max_length[100]|xss_clean');
// Validating Assistant Surgeon Field
        $this->form_validation->set_rules('case_manager_a_s_1', 'Assistant Surgeon (1)', 'trim|min_length[1]|max_length[100]|xss_clean');
// Validating Assistant Surgeon Field
        $this->form_validation->set_rules('case_manager_a_s_2', 'Assistant Surgeon(2)', 'trim|min_length[1]|max_length[100]|xss_clean');
// Validating Anaesthisist Field
        $this->form_validation->set_rules('case_manager_anaes', 'Anaesthisist', 'trim|min_length[2]|max_length[100]|xss_clean');
...
// redirect('finance_/add_new'); 
        } else {
            $case_manager_fees = $c_s_f + $a_s_f_1 + $a_s_f_2 + $anaes_f;
            $account__fees = $ot_charge + $cab_rent + $seat_rent + $others;
            $amount = $case_manager_fees + $account__fees;
            $discount_type = $this->finance_model->getDiscountType();
            ...
            if ($discount_type == 'flat') {
                ...
                $account__fees = $account__fees - $flat_discount;
            } else {
                $flat_discount = $amount * ($discount / 100);
            ...
            if (empty($id)) {
                $data = array(
                    'patient' => $patient,
                    'case_manager_c_s' => $case_manager_c_s,
                    'case_manager_a_s_1' => $case_manager_a_s_1,
                    'case_manager_a_s_2' => $case_manager_a_s_2,
                    'case_manager_anaes' => $case_manager_anaes,
                    'n_o_o' => $n_o_o,
                    'c_s_f' => $c_s_f,
                    'a_s_f_1' => $a_s_f_1,
                    'a_s_f_2' => $a_s_f_2,
                    ...
                    'date' => $date,
                    'amount' => $amount,
                    'case_manager_fees' => $case_manager_fees,
                    'account__fees' => $account__fees,
                    'gross_total' => $gross_total,
                    'flat_discount' => $flat_discount,
                    'amount_received' => $amount_received,
                    'status' => 'unpaid',
    ...
    function editOtPayment() {
        if ($this->ion_auth->in_group(array('admin', 'Finance_'))) {
            $data = array();
            $data['discount_type'] = $this->finance_model->getDiscountType();
            $data['settings'] = $this->settings_model->getSettings();
            $data['patients'] = $this->patient_model->getPatient();
...
// Validating Case_manager Commission Rate Field
        $this->form_validation->set_rules('d_commission', 'Case_manager Commission rate', 'trim|min_length[1]|max_length[100]|xss_clean');
// Validating Description Field
    ...
    function editPaymentCategory() {
        ...
        if (!empty($id)) {
            $payment_category_details = $this->finance_model->getPaymentCategoryById($id);
            if ($payment_category_details->account__id != $this->session->userdata('account__id')) {
                redirect('home/permission');
            }
        }
...
// Validating Note Field
    ...
    function editExpense() {
        ...
        if (!empty($id)) {
            $expense_details = $this->finance_model->getExpenseCategoryById($id);
            if ($expense_details->account__id != $this->session->userdata('account__id')) {
                redirect('home/permission');
            }
        }
...
// Validating Description Field
    ...
    function editExpenseCategory() {
        ...
        if (!empty($id)) {
            ...
            if ($expense_category_details->account__id != $this->session->userdata('account__id')) {
                redirect('home/permission');
            }
        }
    ...
    function invoice() {
        $id = $this->input->get('id');
        $data['payment'] = $this->finance_model->getPaymentById($id);
        $patient_account__id = $this->patient_model->getPatientById($data['payment']->patient)->account__id;
        if ($patient_account__id != $this->session->userdata('account__id')) {
            redirect('home/permission');
        }
        $data['settings'] = $this->settings_model->getSettings();
        $data['discount_type'] = $this->finance_model->getDiscountType();
    ...
    function expenseInvoice() {
        ...
        if (!empty($id)) {
            $expense_invoice_details = $this->finance_model->getExpenseById($id);
            if ($expense_invoice_details->account__id != $this->session->userdata('account__id')) {
                redirect('home/permission');
            }
        }
    ...
    function patientPaymentHistory() {
        ...
        $patient_account__id = $this->patient_model->getPatientById($patient)->account__id;
        if ($patient_account__id != $this->session->userdata('account__id')) {
            redirect('home/permission');
        }
...
// Validating Deposited Amount Field
                ...
                if ($deposit_type == 'Card') {
                    ...
                    if ($gateway == 'PayPal') {
                        ...
                        $all_details = array(
                            ...
                            'case_manager' => $payment_details->case_manager_name,
    ...
    function case_managersCommission() {
        $date_from = strtotime($this->input->post('date_from'));
        $date_to = strtotime($this->input->post('date_to'));
        if (!empty($date_to)) {
            $date_to = $date_to + 86399;
        ...
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('case_managers_commission', $data);
        $this->load->view('home/footer'); // just the footer fi
    }
    ...
    function docComDetails() {
        ...
        $case_manager = $this->input->get('id');
        ...
        if (!empty($date_from)) {
            $data['payments'] = $this->finance_model->getPaymentByCase_managerDate($case_manager, $date_from, $date_to);
        } else {
            $data['payments'] = $this->finance_model->getPaymentByCase_manager($case_manager);
        }
        $data['settings'] = $this->settings_model->getSettings();
        $data['from'] = $this->input->post('date_from');
        $data['to'] = $this->input->post('date_to');
...
// } 
    ...
    function UserActivityReport() {
        ...
        if ($this->ion_auth->in_group(array('Finance_'))) {
            $user = $this->ion_auth->get_user_id();
            $data['user'] = $this->finance__model->getFinance_ByIonUserId($user);
        }
        if ($this->ion_auth->in_group(array('Admin_comm'))) {
            $user = $this->ion_auth->get_user_id();
            $data['user'] = $this->admin_comm_model->getAdmin_commByIonUserId($user);
        }
        $hour = 0;
    ...
    function AllUserActivityReport() {
        ...
        if (!empty($user)) {
            ...
            if ($data['user']->account__id != $this->session->userdata('account__id')) {
                redirect('home/permission');
            }
        ...
        if (empty($user)) {
            ...
            $data['admin_comms'] = $this->admin_comm_model->getAdmin_comm();
            $data['settings'] = $this->settings_model->getSettings();
    ...
    function getPayment() {
        ...
        foreach ($data['payments'] as $payment) {
            ...
            $case_manager_details = $this->case_manager_model->getCase_managerById($payment->case_manager);

            if (!empty($case_manager_details)) {
                $case_manager = $case_manager_details->name;
            } else {
                if (!empty($payment->case_manager_name)) {
                    $case_manager = $payment->case_manager_name;
                } else {
                    $case_manager = $payment->case_manager_name;
                }
            }
