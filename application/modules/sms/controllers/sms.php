...
class Sms extends MX_Controller {
    ...
    public function sendView() {
        ...
        $data['teams'] = $this->case_manager_model->getCase_manager();
        $this->load->view('home/dashboard'); // just the header file
    ...
    function send() {
        ...
        if ($is_v_v == 'allcase_manager') {
            $case_managers = $this->case_manager_model->getCase_manager();
            foreach ($case_managers as $case_manager) {
                $to[] = $case_manager->phone;
            }
            $recipient = 'All Case_manager';
        }

        if ($is_v_v == 'contactgroupwise') {
            $contact_group = $this->input->post('contactgroup');
            $contact_lists = $this->contact_list_model->getContact_list();
            foreach ($contact_lists as $contact_list) {
                if ($contact_list->group == $contact_group) {
                    $to[] = $contact_list->phone;
                }
            }
            $recipient = 'All Blood Contact_lists With Contact Group ' . $contact_group;
        }
    ...
    function patient_careReminder() {
        $id = $this->input->post('id');
        $patient_care_details = $this->patient_care_model->getPatient_careById($id);

        $patient_detail = $this->patient_model->getPatientById($patient_care_details->patient);
        $case_manager_detail = $this->case_manager_model->getCase_managerById($patient_care_details->case_manager);
        ...
        // $message = urlencode("Test Message");
        if (!empty($to)) {
            $message = 'Reminder: Patient_care is scheduled for you With Case_manager ' . $case_manager_detail->name . ' Date: ' . date('d-m-Y', $patient_care_details->date) . ' Time: ' . $patient_care_details->s_time;
            $message1 = urlencode($message);
            $this->sendSms($to, $message1);
            $data_p = array();
            $date = time();
        ...
        redirect('patient_care/upcoming');
    }

    function sendSmsDuringPatient_care($patient, $case_manager, $date, $s_time, $e_time) {

        $patient_detail = $this->patient_model->getPatientById($patient);
        $case_manager_detail = $this->case_manager_model->getCase_managerById($case_manager);

        $recipient_p = 'Patient Id: ' . $patient_detail->id . '<br> Patient Name: ' . $patient_detail->name . '<br> Patient Phone: ' . $patient_detail->phone;
        $recipient_d = 'Case_manager Id: ' . $case_manager_detail->id . '<br> Patient Name: ' . $case_manager_detail->name . '<br> Case_manager Phone: ' . $case_manager_detail->phone;


        // $message = urlencode("Test Message");
        if (!empty($patient)) {
            $message = 'Patient_care is scheduled for you With Case_manager ' . $case_manager_detail->name . ' Date: ' . date('d-m-Y', $date) . ' Time: ' . $s_time;
            $message1 = urlencode($message);
        ...
        if (!empty($case_manager)) {
            $message = 'Patient_care is scheduled for you With Patient ' . $patient_detail->name . ' Date: ' . date('d-m-Y', $date) . ' Time: ' . $s_time;
            $message1 = urlencode($message);
            $this->sendSms($case_manager_detail->phone, $message1);
            $data_d = array();
            $date = time();
            $data_d = array(
                'message' => $message,
    ...
    function patient_careApproved() {
        ...
        // $message = urlencode("Test Message");
        if (!empty($to)) {
            $message = 'Approval: Patient_care is scheduled for you With Case_manager ' . $case_manager_detail->name . ' Date: ' . date('d-m-Y', $patient_care_details->date) . ' Time: ' . $patient_care_details->s_time;
            $message1 = urlencode($message);
            $this->sendSms($to, $message1);
            $data_p = array();
            $date = time();
